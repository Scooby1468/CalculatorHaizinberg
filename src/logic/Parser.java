		...
		for (String item : strArr) {
            ...
            if(parseOperator(item)) {	
	            ...
	            if(isRoman && Converter.isRoman(a) && Converter.isRoman(b)) {
	            	operations.add(new RomanNumber(Converter.romanToInt(a), Converter.romanToInt(b), oprt));
	            	isArabic = false;
	            }
	            else if(isArabic && Converter.isConvertibleToInt(a) && Converter.isConvertibleToInt(b)) {
	            	operations.add(new ArabicNumber(Converter.stringToInt(a), Converter.stringToInt(b), oprt));
	            	isRoman = false;
	            }
	            else {
	            	throw new IllegalArgumentException("The inserted string should contain only roman or only arabic numbers !");
	            }
            }
		}
		return true;
